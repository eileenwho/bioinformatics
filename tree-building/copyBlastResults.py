#to use this script: go down to main, after open_dir_fix( substitute in
# the file path to the folder holding all the files, the name of the directory with the existing fasta files for proteins,
# the name of the directory with the new sequences to add to the existing files
...
def trim_query(defline):
    ...
    for section in defline_split:
        if section == "Query":
            ...
            defline_query_trimmed=defline_query_trimmed[0:-2]
            defline_query_trimmed += "\n"
            break
        else:
...
if __name__ == '__main__':
    ...
    open_dir_fix("C:/Users/Eileen/Documents/2017 UROP with Lily in EAPS/example_files",
                 "Existing fasta files", "New sequences from CCMP_1991", protein_key)
    print('finished')